...
class AscProcessor(DataProcessor):
    ...
    def get_labels(self, mode):
        if mode=='pc':
            import pickle
            with open('../data/pc/product_categories', 'rb') as f:
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, ids) in enumerate(lines):
            guid = "%s-%s" % (set_type, ids )
            text_a = lines[ids]['title']
            if isinstance(text_a, float):
                continue
            # text_b = lines[ids]['term']
            label = eval(lines[ids]['category'])
            # print('es', label, type(label))
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label))
        return examples     
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, mode):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if mode=='pc':
            ...
            # cnt = 0
            for c in example.label:
                # cnt += 1
                label_id[label_map[c]] = 1
            # print('go', cnt)
        elif mode!="ae":
