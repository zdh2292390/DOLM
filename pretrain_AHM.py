...
# The pretraining head is defined in "./transformers/modeling_bert.py"
MODEL_CLASSES = {'bert': (BertConfig, BertForMaskedLM, BertTokenizer),}
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, return_features=False):
    ...
    if os.path.exists(cached_features_file) and (not hasattr(args, 'overwrite_cache') or not args.overwrite_cache):
        ...
        # features, Phrases = pool.starmap_async(convert_examples_to_features, zip(examples,
        #     tokenizer,
        ...
        #   phrase_table= get_phrase_table()['phrase'].unique())).get()
        # pool.close()
        # pool.join()
...
def load_bert_dataset(args, task, tokenizer, evaluate=False, replica=1):
  ...
  for (ft_index, feature) in enumerate(features):
    for _ in range(replica):
      ...
      # word learning
      if Phrases == [] or Phrases[ft_index] == [] or random.uniform(0,1) > 0.5:
      ...
      # phrase learning
        ...
        # covered_indexes = set()
        token_loc = []
        for index in range(len_cand):
          for j, token in enumerate(init_ids):
            if input_ids[j:j+len(cand[index])] == cand[index]:
              token_loc = range(j, j + len(cand[index]))
              break
          if len(masked_lms_pos) >= num_to_predict:
            break
          ...
          masked_token = None
          if rng.random() < 0.8:
            masked_token = [MASK_id for token_idx in token_loc]
          else:
            if rng.random() < 0.5:
              masked_token = [init_ids[token_idx] for token_idx in token_loc]
            else:
              masked_token = [init_ids[rng.randint(0, len(init_ids) - 1)] for token_idx in token_loc]
         
          for idx in range(len(token_loc)):
            token_idx = token_loc[idx]
            masked_lm_labels[token_idx] = init_ids[token_idx]
            input_ids[token_idx] = masked_token[idx]
            masked_lms_pos.append(token_idx)
